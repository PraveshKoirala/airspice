...
import type { Analysis, Component, FirmwareOperation, SystemIR, Test } from "../model.js";
...
import { SPICE_MODELS } from "../registry/index.js";
 ...
 * A part-level `spice_model` on one of these is what makes an imported model card
 * (SPICE_MODELS) get emitted for the design (issue #60). Mirror of spice.py
 * `_MODELLED_SPICE_TYPES` / validation's set.
 */
const MODELLED_SPICE_TYPES: ReadonlySet<string> = new Set(["bjt", "mosfet", "diode"]);

/**
 * The real `.model` cards for every imported part the design references, mirror
 * of spice.py `_imported_model_cards`. A component of a modelled type whose
 * `spice_model` names a part in the imported model library (SPICE_MODELS)
 * contributes that part's stored card; cards are de-duplicated and emitted in
 * sorted model-name order so the netlist is deterministic and byte-stable. The
 * card text is the SAME string the Python oracle stores (both read the generated
 * registry), so the emitted `.model` bytes are identical. A design referencing no
 * imported part yields `[]`, leaving every existing corpus netlist unchanged.
 */
function importedModelCards(ir: SystemIR): string[] {
  // PARITY: the card strings live in the generated registry (gen-registry.mjs
  // copies registry/imported/*.json verbatim, the same files registry.py reads),
  // so this port never re-normalizes model text -- byte-identity with the Python
  // oracle is structural. Only the SELECTION (modelled type + referenced +
  // sorted + de-duplicated) is reproduced here, matching spice._imported_model_cards.
  const names = new Set<string>();
  for (const component of ir.components.values()) {
    const model = component.spice_model;
    if (model && MODELLED_SPICE_TYPES.has(component.type)) {
      const key = model.trim().toUpperCase();
      if (Object.prototype.hasOwnProperty.call(SPICE_MODELS, key)) names.add(key);
    }
  }
  return [...names].sort(pyStrCmp).map((name) => SPICE_MODELS[name]!.card);
}
...
export function compileSpice(
  ...
  // verbatim so every existing corpus/ground-truth design is unchanged. Byte-
  // parity mirror of the Python oracle in packages/core/src/air/spice.py.
  const acAnalysis: Analysis | null =
    test && test.analysis && test.analysis.type === "ac" ? test.analysis : null;

  const lines: string[] = [
    "* Generated by AIR",
    ".options filetype=ascii",
    ...BUILTIN_MODEL_CARDS,
    ...importedModelCards(ir),
  ];
  ...
  for (const [sourceName, [net, value]] of voltageSources) {
    ...
    let line = `${sourceName} ${spiceNet(net)} 0 DC ${spiceValue(value)}`;
    if (acAnalysis !== null) {
      // Test-driven sources carry no AC magnitude declaration; default 0 so the
      // source acts as a pure bias under the AC solve (byte parity with the
      // oracle).
      line += " AC 0";
    }
    lines.push(line);
  ...
  for (const component of sortedComponents(ir)) {
    const emitted = componentLine(component, test ?? null, acAnalysis !== null);
    if (emitted) {
      lines.push(emitted);
  ...
  if (acAnalysis !== null) {
    ...
    // Frequencies are converted to plain Hz numeric strings so ngspice never
    // sees an SI suffix in this position, matching the oracle's ``%g``
    // rendering byte-for-byte.
    const startHz = formatHz(parseQuantity(acAnalysis.start, "Hz"));
    const endHz = formatHz(parseQuantity(acAnalysis.end, "Hz"));
    lines.push(
      `.ac ${acAnalysis.sweep} ${acAnalysis.points} ${startHz} ${endHz}`,
      ".control",
      "run",
    );
  } else {
  ...
  if (test) {
    ...
    if (acAnalysis !== null) {
      // AC-flavoured probes come from assert_gain_db_at_freq AND assert_voltage
      // (a design mixing the two under one AC analysis still probes both).
      for (const a of test.assertions) {
        const op = a["op"];
        if ((op === "assert_gain_db_at_freq" || op === "assert_voltage") && a["net"]) {
    ...
    for (const net of probeNets) {
      const sNet = spiceNet(net);
      if (acAnalysis !== null) {
        // Magnitude(dB) and phase(rad) per frequency point. ngspice's ``wrdata``
        // repeats the frequency column per variable (§15.6 manual) so each row
        // is ``freq vdb freq vphase``; the simulator parses that into a
        // frequency response and converts vphase from radians to degrees.
        lines.push(
          `wrdata ../waveforms/${test.id}_${net}.csv vdb(${sNet}) vp(${sNet})`,
        );
      } else {
        lines.push(`wrdata ../waveforms/${test.id}_${net}.csv v(${sNet})`);
      }
    }
  }
...
/**
 * Render a frequency in Hz as an ngspice-parseable number (no SI suffix), byte-
 * for-byte with the Python oracle's ``%g``. JS's default number formatting
 * omits the decimal point on integers and uses ``e+XX`` form for magnitudes
 * ``>= 1e21`` -- which does not match Python's ``%g`` in the corpus range
 * (10 Hz .. 1e12 Hz). We reproduce ``%g`` by hand for the ranges the AC card
 * uses today (positive finite frequencies), so oracle and port emit identical
 * bytes for the ``.ac`` line.
 */
function formatHz(valueHz: number): string {
  // Python ``%g`` with default precision 6: use significant digits, drop
  // trailing zeros, drop trailing decimal point, and switch to exponential
  // form when the exponent < -4 or >= precision (6). Non-finite / non-positive
  // inputs cannot appear in a validated .ac card so a plain fallback suffices.
  if (!isFinite(valueHz) || valueHz <= 0) {
    return String(valueHz);
  }
  const precision = 6;
  const exp = Math.floor(Math.log10(valueHz));
  if (exp < -4 || exp >= precision) {
    // Scientific: {mantissa}e+{exp}.
    const mantissa = valueHz / Math.pow(10, exp);
    let mStr = mantissa.toPrecision(precision);
    // Strip trailing zeros after decimal, and trailing decimal point.
    if (mStr.indexOf(".") >= 0) {
      mStr = mStr.replace(/0+$/, "").replace(/\.$/, "");
    }
    const sign = exp >= 0 ? "+" : "-";
  ...
  // Fixed: precision significant digits, strip trailing zeros / point.
  let s = valueHz.toPrecision(precision);
  if (s.indexOf(".") >= 0) {
 ...
 * (`NPN` / `NMOS` / `D`).
 */
function componentLine(component: Component, test: Test | null, acMode = false): string | null {
  ...
  if (component.type === "voltage_source" && component.value && pins.length >= 2) {
    let line = `V_${component.id} ${spiceNet(pins[0]!.net)} ${spiceNet(pins[1]!.net)} DC ${spiceValue(component.value)}`;
    if (acMode) {
      // AC small-signal magnitude (issue #62). Sources without an explicit
      // ac_magnitude get ``AC 0`` so they act as pure biases under the .ac
      // solve, mirroring the oracle byte-for-byte.
      const acMag = component.properties.get("ac_magnitude") ?? "0";
      line += ` AC ${spiceValue(acMag)}`;
    }
    return line;
  }
