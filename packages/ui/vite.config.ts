...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      // transpiles the .ts entry directly (issue #10 / #17 / #13). Same effect
      // as consuming the built dist/, but with per-file HMR in `vite dev`.
      // Node-side resolution (Vitest, tsc -b in agent/air-ts, etc.) uses the
      // real workspace symlink -- these aliases only bind inside Vite.
  ...
  server: {
    fs: {
      ...
      // agent, sim-wasm) whose SOURCE we consume via the aliases above. Vite's
      // dev server restricts @fs reads to the project root by default; the
      // sim-wasm engine worker (packages/sim-wasm/src/engine.worker.ts) lives
      // outside it, so the dev-only /sim-lab route needs the monorepo
      // packages/ dir allow-listed.
